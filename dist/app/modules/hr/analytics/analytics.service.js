...
const attendance_model_1 = require("../../../../shared/models/attendance.model");
const leave_model_1 = require("../../../../shared/models/leave.model");
class AnalyticsService {
    /**
     * Get comprehensive attendance statistics
     */
    async getAttendanceStatistics(month, year, departmentId) {
        // Current month stats
        ...
        const attendanceChange = this.calculatePercentageChange(previousStats.avgAttendanceRate, currentStats.avgAttendanceRate);
        const workingHoursChange = this.calculateAbsoluteChange(previousStats.avgWorkingHours, currentStats.avgWorkingHours);
        const overtimeChange = this.calculatePercentageChange(previousStats.totalOvertimeHours, currentStats.totalOvertimeHours);
        const leavesChange = this.calculatePercentageChange(previousStats.leavesTaken, currentStats.leavesTaken);
        return {
     ...
     */
    async calculateMonthStats(month, year, departmentId) {
        const startDate = new Date(year, month - 1, 1);
        const endDate = new Date(year, month, 0);
        // Get all active employees
        const employeeFilters = { isActive: true };
        if (departmentId) {
            employeeFilters['professionalDetails.department'] = departmentId;
        }
        const employees = await user_dal_1.userDAL.findAll(employeeFilters, { limit: 10000 });
        const totalEmployees = employees.total;
        // Get attendance records for the month
        const attendanceFilters = {
            date: { $gte: startDate, $lte: endDate }
        };
        const attendanceRecords = await attendance_model_1.AttendanceModel.find(attendanceFilters);
        // Calculate working days in month
        const workingDays = this.getWorkingDaysInMonth(year, month);
        // Calculate metrics
        const totalPossibleAttendance = totalEmployees * workingDays;
        const totalPresent = attendanceRecords.filter(r => ['PRESENT', 'LATE', 'WFH'].includes(r.status)).length;
        // Avg Attendance Rate
        const avgAttendanceRate = (totalPresent / totalPossibleAttendance) * 100;
        // Avg Working Hours
        const totalWorkingHours = attendanceRecords.reduce((sum, record) => sum + (record.workingHours || 0), 0);
        const avgWorkingHours = totalWorkingHours / (totalPresent || 1);
        // Get average shift minimum hours for comparison
        const users = await user_dal_1.userDAL.findAll(departmentId ? { 'professionalDetails.department': departmentId, isActive: true } : { isActive: true }, { limit: 10000 });
        ...
        // Leaves Taken
        const leaveRecords = await leave_model_1.LeaveModel.find({
            startDate: { $gte: startDate, $lte: endDate },
            status: 'APPROVED'
        });
        const leavesTaken = leaveRecords.reduce((sum, leave) => sum + leave.numberOfDays, 0);
        return {
     ...
     */
    calculateAbsoluteChange(oldValue, newValue) {
     ...
     */
    async getDepartmentWiseAnalytics(month, year) {
        const departments = await user_dal_1.userDAL.findAll({ isActive: true }, { limit: 10000 });
     ...
     */
    async getEmployeePerformanceAnalytics(userId, month, year) {
        const startDate = new Date(year, month - 1, 1);
        const endDate = new Date(year, month, 0);
        // Get attendance records
        const attendanceRecords = await attendance_model_1.AttendanceModel.find({
            userId,
            date: { $gte: startDate, $lte: endDate }
        });
        const workingDays = this.getWorkingDaysInMonth(year, month);
        const presentDays = attendanceRecords.filter(r => ['PRESENT', 'LATE', 'WFH'].includes(r.status)).length;
        ...
        // Get leaves
        const leaves = await leave_model_1.LeaveModel.find({
            userId,
            startDate: { $gte: startDate, $lte: endDate },
            status: 'APPROVED'
        });
     ...
     */
    async getAttendanceTrend(year, departmentId) {
        const monthlyData = [];
     ...
     */
    async getLeaveStatistics(month, year, departmentId) {
        const startDate = new Date(year, month - 1, 1);
        const endDate = new Date(year, month, 0);
        const filters = {
            startDate: { $gte: startDate, $lte: endDate }
        ...
        const filteredLeaves = departmentId
            ? leaves.filter(l => l.userId.professionalDetails?.department?.toString() === departmentId)
            : leaves;
        // Group by leave type
        const leavesByType = filteredLeaves.reduce((acc, leave) => {
        ...
        // Group by status
        const leavesByStatus = filteredLeaves.reduce((acc, leave) => {
        ...
        const totalEmployees = await user_dal_1.userDAL.findAll(departmentId ? { 'professionalDetails.department': departmentId, isActive: true } : { isActive: true }, {});
    ...
    async getRealTimeDashboardStats() {
        ...
        const todayAttendance = await attendance_model_1.AttendanceModel.find({
            date: { $gte: todayStart, $lte: todayEnd }
        });
        const totalEmployees = await user_dal_1.userDAL.findAll({ isActive: true }, {});
        const presentToday = todayAttendance.filter(r => ['PRESENT', 'LATE', 'WFH'].includes(r.status)).length;
        ...
        const pendingLeaves = await leave_model_1.LeaveModel.countDocuments({ status: 'PENDING' });
        return {
            today: {
